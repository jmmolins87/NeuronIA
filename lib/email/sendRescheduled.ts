...
import { buildRescheduledEmail } from "@/lib/email/templates/rescheduled"
...
export async function sendRescheduledEmail(args: {
  ...
  if (!env.BREVO_API_KEY || !env.EMAIL_FROM) {
    await writeEventSafe({
      ...
      type: "EMAIL_RESCHEDULE_SEND_FAIL",
  ...
  const fromSchedule = args.fromBooking
    ? {
        date: formatZonedYYYYMMDD(args.fromBooking.startAt, bookingConfig.timeZone),
        startTime: formatZonedHHmm(args.fromBooking.startAt, bookingConfig.timeZone),
        endTime: formatZonedHHmm(args.fromBooking.endAt, bookingConfig.timeZone),
      }
    : undefined
  ...
  const template = buildRescheduledEmail({
    locale,
    appUrl,
    booking: {
      id: args.booking.id,
  ...
  await writeEventSafe({
    bookingId: args.booking.id,
    type: "EMAIL_RESCHEDULE_SEND_ATTEMPT",
  ...
  try {
    ...
    console.error("[email] rescheduled send failed", {
      bookingId: args.booking.id,
      provider,
