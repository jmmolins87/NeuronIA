...
export default function ReservarPage() {
  ...
  const handleBookingComplete = React.useCallback(
    (bookingData: BookingCompleteData) => {
  ...
  // Interceptar clicks en enlaces para mostrar modal (sin beforeunload nativo)
  React.useEffect(() => {
              ...
              {hasSubmittedBefore ? (
                <div className="rounded-xl border border-blue-500/50 bg-card/80 backdrop-blur-sm p-8 text-center">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 justify-center">
                    <DemoButton onClick={handleGoToHome}>{t("booking.alreadySubmitted.goBack")}</DemoButton>
                    <DemoButton onClick={handleGoToContact}>{t("booking.alreadySubmitted.goToContact")}</DemoButton>
                  </div>
                </div>
              ) : (
                <div className="backdrop-blur-sm overflow-hidden mb-8 p-6">
                  <BookingCalendar onBookingComplete={handleBookingComplete} onDateSelected={() => setHasInteracted(true)} />
                </div>
              )}
            </Reveal>
          </div>
      ...
      <AlertDialog open={showLeaveWarning} onOpenChange={setShowLeaveWarning}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t("booking.leaveWarning.description")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
